  ...
  return (
    <div className="p-4 sm:p-6 max-w-7xl">
      ...
      <div className="space-y-3 relative">
            ...
            return (
              <div key={fn.id} className="rounded-xl p-3" style={{ background: 'var(--color-surface-card)', border: '1px solid var(--color-border-dim)' }}>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="type-2xs type-mono" style={{ color: 'var(--color-text-muted)' }}>({allSubs.length})</span>
                </div>
                <div className="flex flex-wrap gap-1">
                  {allSubs.map(sub => {
                    ...
                    return (
                      <button key={sub.id} type="button" aria-label={`${sub.id}: ${MATURITY_LABELS[maturity]}`} className="w-6 h-6 rounded cursor-pointer hover:ring-2 hover:ring-slate-400 hover:z-10 focus-visible:ring-2 focus-visible:ring-cyan-400" style={{ backgroundColor: MATURITY_HEX[maturity] }}
        ...
        {tooltip && (
          <div className="fixed z-50 rounded-lg px-3 py-2 type-sm pointer-events-none max-w-xs" style={{ left: tooltip.x, top: tooltip.y, transform: 'translate(-50%, -100%)', background: 'var(--color-surface-overlay)', border: '1px solid var(--color-border-bright)', boxShadow: '0 8px 32px rgba(0,0,0,0.5)' }}>
            <p className="type-mono font-semibold" style={{ color: 'var(--color-accent)' }}>{tooltip.id}</p>
